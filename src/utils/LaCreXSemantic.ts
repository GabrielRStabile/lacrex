...
export default class LaCrexSemantic {
  ...
  private static verifyAtribuicao(atribuicao: AtribuicaoContext): void {
    ...
    if (!variableNameNode) {
        console.error("Erro semântico: Não foi possível encontrar o nome da variável na atribuição.");
        return;
    }
    ...
    if (variableType && this.typeMap.hasOwnProperty(variableType)) {
      ...
      console.error(`Erro semântico: Tipo da variável '${variableName}' não reconhecido ou indefinido.`);
  }
    ...
    if (!this.isVariableDeclared(variableName)) {
      if (variableType) {
        ...
        console.error(`Erro semântico: Identificador '${variableName}' utilizado antes da declaração.`);
        return;
      }
    }
    ...
    if (!declaredType || !(declaredType in this.typeMap)) {
      console.error(`Erro semântico: O tipo de '${variableName}' não foi declarado ou é desconhecido.`);
      return;
  }
    ...
    if (this.typeMap[declaredType] !== valorType) {
      console.error(`Erro semântico: O tipo do valor atribuído a '${variableName}' (${valorType}) não condiz com o tipo da variável (${this.typeMap[declaredType]}).`);
    }
  }
